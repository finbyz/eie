...
def send_sales_invoice_mails():
    ...
    # Changes added as per Mr. Mukesh Requirement - 20-12-2023
    ...
    for customer in customers:
        ...
        with open(path, "rb") as f:
            filedata = f.read()
        attachments.append({"fname": file.file_name, "fcontent": filedata})
...
def contact_validate(self,method):
    for email in self.email_ids:
        exist = frappe.db.get_all("Contact Email",{"parenttype":"Contact","email_id":email.email_id,"parent":("!=",self.name)},"parent")
        for parent in exist:
