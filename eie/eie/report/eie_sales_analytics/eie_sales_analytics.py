...
class Analytics(object):
	...
	def get_data(self):
		...
		elif self.filters.tree_type == 'Industry Type':
			self.get_sales_transactions_based_on_industry()
			self.get_rows()

	def get_sales_transactions_based_on_industry(self):
		...
		self.entries = frappe.db.sql("""
			select cus.industry as entity, doc.{date_field}, sum(doc.{value_field}) as value_field 
			from `tab{doctype}` doc LEFT JOIN `tabCustomer` cus on (doc.customer = cus.name)
			where doc.docstatus = 1 and doc.company = %s
			and doc.{date_field} between %s and %s
			group by cus.industry, doc.{date_field}
		...
		for d in self.entries:
			self.entity_names.setdefault(d.entity, d.entity)
