...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for d in item_list:
		...
		row.append(d.gst_hsn_code)

		row += [
			d.customer_group, d.debit_to, ", ".join(mode_of_payments.get(d.parent, [])),
			d.territory, d.project, d.company, d.sales_order,
			delivery_note, d.income_account, d.cost_center, d.quan, d.base_net_rate, d.base_net_amount
		]
...
def get_items(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select
			...
			si_item.delivery_note, si_item.income_account, si_item.cost_center,SUM(si_item.qty) as quan,
			SUM(si_item.base_net_rate) as base_net_rate, SUM(si_item.base_net_amount) as base_net_amount, 
			si.customer_name,si.customer_group, si_item.so_detail, si.update_stock ,si_item.gst_hsn_code {0}
		from `tabSales Invoice` si, `tabSales Invoice Item` si_item
		where si.name = si_item.parent and si.docstatus = 1 %s
		group by si_item.item_code,si_item.gst_hsn_code
