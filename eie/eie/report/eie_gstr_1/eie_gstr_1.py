...
class Gstr1Report(object):
	def __init__(self, filters=None):
		...
		self.select_columns = """
			...
			invoice_type,
			return_against,
			(select si.posting_date from `tabSales Invoice` si where si.name = `tabSales Invoice`.return_against) as return_invoice_date,
			is_return,
			invoice_type,
			export_type,
			port_code,
	...
	def run(self):
		...
		if self.invoices:
			...
			frappe.errprint(str(self.items_based_on_tax_rate))
	...
	def get_data(self):
		for inv, items_based_on_rate in self.items_based_on_tax_rate.items():
			...
			for rate, items in items_based_on_rate.items():
				...
				if self.filters.get("type_of_business") in ["CDNR", "CDNUR"]:
	...
	def get_row_data_for_invoice(self, invoice, invoice_details, tax_rate, items):
		...
		for fieldname in self.invoice_fields:
			if self.filters.get("type_of_business") in ["CDNR", "CDNUR"] and fieldname == "invoice_value":
			...
			if item_code in items:
				...
				for tax in taxes:
					if tax.account_head not in taxes_list[0].values():
						...
						taxable_value += abs(item_wise_tax.get(item_code)[1])

		row += [tax_rate, taxable_value, None]
	...
	def get_conditions(self):
		...
		if self.filters.get("type_of_business") ==  "B2B":
			conditions += """ and ifnull(invoice_type, '') != 'Export' and customer_gstin is not NULL and is_return != 1
		...
		if self.filters.get("type_of_business") ==  "B2C Large":
			conditions += """ and customer_gstin is NULL and base_net_total > {0}
				and is_return != 1 and customer in ('{1}')""".\
			...
			conditions += """ and is_return = 1 and customer_gstin is not NULL """

		elif self.filters.get("type_of_business") ==  "CDNUR":
			conditions += """ and is_return = 1 and customer_gstin is NULL """

		elif self.filters.get("type_of_business") ==  "EXPORT":
			conditions += """ and is_return != 1 and invoice_type = 'Export' """

		elif self.filters.get("type_of_business") ==  "SEZ":
			conditions += """ and is_return != 1 and invoice_type = 'SEZ' """
	...
	def get_items_based_on_tax_rate(self):
				...
				if item_wise_tax_detail:
					try:
						...
						if not (cgst_or_sgst or account in self.gst_accounts.igst_account):
							...
							if self.filters.get("type_of_business") not in ["EXPORT", "SEZ"]:
								continue
		...
		if self.filters.get("type_of_business") in ["EXPORT", "SEZ"]:
			tax_keys = self.items_based_on_tax_rate.keys()
			inv = [d for d in self.invoice_items.keys() if d not in tax_keys]

			for i in inv:
				self.items_based_on_tax_rate.setdefault(i, {}).setdefault(0, [])
	...
	def get_columns(self):
		...
		elif self.filters.get("type_of_business") in ["CDNR", "CDNUR"]:
			self.invoice_columns = [
				{
					"fieldname": "customer_gstin",
					"label": "GSTIN/UIN of Recipient",
		...
		elif self.filters.get("type_of_business") in ["EXPORT", "SEZ"]:
			self.invoice_columns = [
				{
