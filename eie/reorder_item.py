...
def _reorder_item():
	...
	lft, rgt = frappe.db.get_value("Item Group", {'name' : 'RAW MATERIAL', 'is_group' : 1, 'parent_item_group' : 'All Item Groups'}, ['lft', 'rgt'])
	items_to_consider = frappe.db.sql_list(f"""select name from `tabItem` item
		where is_stock_item=1 and has_variants=0
			and disabled=0
			and item.item_group in (select name from `tabItem Group` ig \
				where ig.lft < {lft} or ig.rgt > {rgt} and item.item_group = ig.name)
			and (end_of_life is null or end_of_life='0000-00-00' or end_of_life > '{nowdate()}')
	...
	rm_items_to_consider = frappe.db.sql_list(f"""select name from `tabItem` item
		where is_stock_item=1 and has_variants=0
			and disabled=0
			and item.item_group in (select name from `tabItem Group` ig \
				where ig.lft >= {lft} and ig.rgt <= {rgt} and item.item_group = ig.name)
			and (end_of_life is null or end_of_life='0000-00-00' or end_of_life > '{nowdate()}')
			and (exists (select name from `tabItem Reorder` ir where ir.parent=item.name)
				or (variant_of is not null and variant_of != ''
				and exists (select name from `tabItem Reorder` ir where ir.parent=item.variant_of))
	...
	if rm_items_to_consider:
		material_requests = {"Purchase": {}, "Transfer": {}, "Material Issue": {}, "Manufacture": {}}
		item_warehouse_projected_qty = get_item_warehouse_projected_qty(rm_items_to_consider)
		for item_code in rm_items_to_consider:
			item = frappe.get_doc("Item", item_code)
...
def notify_errors(exceptions_list):
	...
	content += ("Regards,") + "<br>" + ("Administrator")
